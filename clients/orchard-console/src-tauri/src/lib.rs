...
fn run_oidc_loopback_login(
    ...
    let mut authorize_url =
        Url::parse(&discovery.authorization_endpoint).map_err(|err| err.to_string())?;
    authorize_url
        .query_pairs_mut()
...
fn open_system_browser(url: &str) -> Result<(), String> {
    ...
    {
        Command::new("cmd")
            .args(windows_start_command_args(url))
            .spawn()
            .map_err(|err| err.to_string())?;
        return Ok(());
    }
...
fn windows_start_command_args(url: &str) -> Vec<String> {
...
#[cfg(test)]
mod tests {
    use super::windows_start_command_args;

    #[test]
    fn windows_start_command_args_quotes_authorization_url() {
        let url =
            "https://issuer.example.com/auth?client_id=desktop&state=abc123&code_challenge=xyz789";
        let args = windows_start_command_args(url);
